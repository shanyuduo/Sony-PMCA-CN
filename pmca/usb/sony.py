 ...
 def writeGpsData(self, file):
  ...
  while remaining > 0:
   ...
   if (remaining == 0 and response != b'\x01\0') or (remaining > 0 and response != b'\0\0'):
    raise Exception('无效的响应：%s' % response)
...
class SonyUpdaterCamera(object):
 ...
 def _sendCommand(self, command, data=b'', bufferSize=BUFFER_SIZE):
  ...
  if responseHeader.responseId != self.ERR_OK:
   ...
   raise Exception('响应错误：0x%x' % responseHeader.responseId)
 ...
 def _sendWriteCommands(self, command, file, size, complete=None):
  ...
  while True:
   ...
   if status == [self.STAT_OK]:
    ...
    raise Exception('固件更新错误：' + ', '.join([self._statusToStr(s) for s in status if s != self.STAT_OK]))
...
class SonyAppInstallCamera(object):
 ...
 def receive(self):
  ...
  if type == self.SONY_MSG_Common:
   ...
   if header.type == self.SONY_MSG_Common_Hello:
    ...
    raise Exception('相机已断开连接')
   else:
    raise Exception('未知的通用消息类型：0x%x' % header.type)
   ...
   if header.type == self.SONY_MSG_Tcp_ProxyConnect:
    ...
    raise Exception('未知的 TCP 消息类型：0x%x' % header.type)
   ...
   if header.type == self.SONY_MSG_Rest_Out:
    ...
    raise Exception('未知的 REST 消息类型：0x%x' % header.type)

  else:
   raise Exception('未知的消息类型：0x%x' % type)
 ...
 def _receiveResponse(self, type):
  ...
  if not isinstance(msg, type):
   raise Exception('错误的响应：%s' % str(msg))
  return msg
...
class SonySenserAuthDevice(SonyUsbDevice):
 ...
 def authenticate(self):
  ...
  res = parse8(data[:1])
  if res != 1:
   raise Exception('Senser 错误 %d' % res)
...
class SonySenserCamera(object):
 ...
 def _sendProductInfoPacket(self, category, command, data=b''):
  ...
  if res != 1:
   raise Exception('Senser 产品信息错误 %d' % res)
  return data
 ...
 def _sendAdjustControlPacket(self, block, code, data=b''):
  ...
  if res != 0:
   raise Exception('Senser 调整控制错误 %d' % res)
  return data
 ...
 def _sendFileControlPacket(self, cmd, filename, data=b'', outFile=None):
  ...
  if res != 1:
   raise Exception('Senser 文件控制错误 %d' % res)
  return data
 ...
 def _sendMemoryDumpPacket(self, base, size, data=b'', outFile=None):
  ...
  if res not in [0, 1]:
   raise Exception('Senser 内存转储错误 %d' % res)
  return data
 ...
 def readBackup(self, id):
  return self._sendAdjustControlPacket(*self.SONY_ADJUST_BACKUP_READ, dump32le(id))
 ...
 def getBackupPresetDataStatus(self):
  return self._sendAdjustControlPacket(*self.SONY_ADJUST_BACKUP_PDT_STAT)
