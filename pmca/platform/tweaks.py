...
class LanguageTweak(BackupTweak):
 ...
 def strValue(self):
  ...
  return '已激活 %d / %d 种语言' % (sum(1 if l == self.BACKUP_LANG_ENABLED else 0 for l in val), len(val))
...
class TweakInterface:
 def __init__(self, backend):
  ...
  self.addTweak('recLimit', '禁用视频录制时长限制', RecLimitTweak(backup))
  self.addTweak('recLimit4k', '禁用 4K 视频录制时长限制', RecLimit4kTweak(backup))
  self.addTweak('language', '解锁所有语言', LanguageTweak(backup))
  self.addTweak('palNtscSelector', '启用 PAL / NTSC 选择器及警告', BooleanBackupTweak(backup, 'palNtscSelector'))
  self.addTweak('usbAppInstaller', '启用 USB 应用安装器', BooleanBackupTweak(backup, 'usbAppInstaller'))
