...
class CameraShell(Shell):
 def __init__(self, backend):
  super(CameraShell, self).__init__("平台 Shell")
  self.backend = backend

  self.addCommand('info', Command(self.info, (), '打印设备信息'))

  if isinstance(self.backend, ShellPlatformBackend):
   self.addCommand('shell', Command(self.shell, (), '启动交互式 Shell'))

  if isinstance(self.backend, FilePlatformBackend):
   self.addCommand('push', Command(self.push, (2,), '将指定文件从电脑复制到设备', '<LOCAL> <REMOTE>'))
   self.addCommand('pull', Command(self.pull, (1, 1, ['.']), '将指定文件从设备复制到电脑', '<REMOTE> [<LOCAL>]'))

  if isinstance(self.backend, BootloaderPlatformBackend):
   self.addCommand('bootloader', Command(self.bootloader, (0, 1, ['.']), '导出引导加载程序', '[<OUTDIR>]'))

  if isinstance(self.backend, MemoryPlatformBackend):
   self.addCommand('bootrom', Command(self.bootrom, (0, 1, ['.']), '导出引导 ROM', '[<OUTDIR>]'))

  if isinstance(self.backend, AndroidPlatformBackend):
   self.addCommand('install', Command(self.install, (1,), '安装指定的 Android 应用', '<APKFILE>'))

  if isinstance(self.backend, BackupPlatformBackend):
   self.addCommand('tweak', Command(self.tweak, (), '调整设备设置'))

   bk = SubCommand()
   bk.addCommand('r', Command(self.readBackup, (1,), '读取备份属性', '<ID>'))
   bk.addCommand('w', ResidueCommand(self.writeBackup, 1, '写入备份属性', '<ID> <DATA>'))
   bk.addCommand('patch', ResidueCommand(self.patchBackup, 1, '通过修补备份文件写入备份属性', '<ID> <DATA>'))
   bk.addCommand('s', Command(self.syncBackup, (), '同步备份数据到磁盘'))
   bk.addCommand('lock', Command(self.lockBackup, (), '锁定受保护的备份设置'))
   bk.addCommand('unlock', Command(self.unlockBackup, (), '解锁受保护的备份设置'))
 ...
 def bootloader(self, localPath='.'):
  if not os.path.isdir(localPath):
   raise Exception('%s 不是目录' % localPath)
 ...
 def tweak(self):
  ...
  while True:
   ...
   try:
    while True:
     try:
      i = int(input('输入要切换的设置项编号（0 表示应用）：'))
      if 0 <= i <= len(tweaks):
       break
     except ValueError:
      pass
